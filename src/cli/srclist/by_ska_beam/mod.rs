...
#[derive(Parser, Debug)]
pub struct SrclistBySkaBeamArgs {
...
fn by_ska_beam(
    ...
    let metadata = if let Some(metafits) = metafits {
        ...
        ska_beam_params = SkaBeamParams {
            ...
            feed_angles_rad: Some(obs_context.feed_angles.clone()),
            feed_coordinates: Some(obs_context.feed_coordindates.clone()),
            ecef_to_local_mats: Some(obs_context.ecef_to_local_mats.clone()),
        };
